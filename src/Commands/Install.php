...
class Install extends BaseCommand
{
     ...
     * @var string
     */
    protected $signature = 'faq:install {--y|y : Skip question?} {--views : Also publish tyondo faq views.} {--f|force : Overwrite existing files.}';
     ...
     * @var string
     */
    protected $description = 'Tyondo faq install wizard';
    ...
    public function handle()
    {
            ...
            $this->comment(PHP_EOL.'Welcome to the Tyondo faq Install Wizard! You\'ll be up and running in no time...');

            try {
                $this->comment(PHP_EOL.'Publishing assets...');
                // Publish config files...
                Artisan::call('faq:publish:config', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(1);
                ...
                Artisan::call('faq:publish:assets', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(2);
                ...
                Artisan::call('faq:publish:packages', [
                    '--y' => true,
                    '--force' => false,
                ]);
                $this->progress(3);
                ...
                // Publish view files...
                if ($withViews) {
                    Artisan::call('faq:publish:views', [
                        '--y' => true,
                        '--force' => $force,
                    ]);
                }
                ...
                $this->info(PHP_EOL.'Adding faq routes to routes/web.php');
                $this->progress(5);
                File::append(
